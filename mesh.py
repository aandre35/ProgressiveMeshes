...
class Mesh:
    ...
    def init_size(self):
        self.sizes = {"v": self.get_nb_vertices(), "f": self.get_nb_faces(), "ev": 0, "efv": 0}

    def add_size(self, key):
        self.sizes[key] += 1

    def get_octet_size(self):
        return self.sizes["v"] * 13 + self.sizes["f"] * 4 + self.sizes["ev"] * 14 + self.sizes["v"] * 4